...
def main(args):
    ...
    classifier = Classifier(label_list=label_list, device=device, cache_dir=args.cache, attacker=Attacker(args),epsilon=args.epsilon)
    ...
    for epoch in range(args.epochs):
        ...
        if do_test:
            ...
            classifier.save(os.path.join(save_dir, f'model_{epoch+1}.pth'))
    ...
    with open(os.path.join(group_dir, f'{args.name}.log'), 'a+') as f:
        f.write(f'{(100. * final_test_acc):.4f}\t')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--norm_type', default='linf', type=str, choices=['l2', 'linf'])
