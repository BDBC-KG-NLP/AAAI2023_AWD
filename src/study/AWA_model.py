...
class AWAClassifier:
    def __init__(self, label_list, device, cache_dir, reg, gamma, adv_rd, fix_adv=False):
    ...
    def freeze_adv(self):
        self.adv_net.requires_grad = False
    ...
    def train_epoch(self):
        ...
        for step, batch in enumerate(tqdm(self._data_loader['train'],
            ...
            for _ in range(self._rd):
                ...
                alpha = alpha * batch[1].unsqueeze(-1) # attention mask
                # print(alpha.squeeze()[0,:5])
                reg_loss = torch.mean(torch.sum(alpha.squeeze(-1), dim=1) / sentence_length)
                ...
                print('#', _, ' adving: ', outputs_adv.item(), reg_loss.item())
            ...
            print(alpha.squeeze()[0,:15], 'loss:', outputs[0].item(), 'adv_loss:', outputs_adv.item())
            self._optimizer.zero_grad()
            loss = outputs_adv
            loss.backward()
            self._optimizer.step()
    ...
    def evaluate(self, set_type):
        ...
        for batch in tqdm(data_loader,
            ...
            with torch.no_grad():
                ...
                print('train loss:', tmp_eval_loss)
    ...
    def evaluate_adv(self, set_type):
        self._model.eval()
        self.adv_net.eval()
