...
class Classifier:
    ...
    def train_epoch(self):
        ...
        for step, batch in enumerate(tqdm(self._data_loader['train'],
            ...
            mixed_embedding, y_a, y_b, lam = mixup_data(bert_embedding, batch[3])
            ...
            # print(outputs)
            loss = outputs[0]
            # print('loss:', loss, 'output[1]', outputs[1].shape, outputs[1])
            mix_loss = mixup_criterion(nn.CrossEntropyLoss(), outputs[1], y_a, y_b, lam)
            mix_loss.backward()
            self._optimizer.step()
